...
import (
	...
	types "github.com/DelosIsland/core/dngine/types"
	"github.com/DelosIsland/core/module/lib/go-crypto"
	rpc "github.com/DelosIsland/core/module/lib/go-rpc/server"
...
func (n *Node) rpcRoutes() map[string]*rpc.RPCFunc {
	...
	return map[string]*rpc.RPCFunc{
		...
		"get_account_amount": rpc.NewRPCFunc(h.GetAccountAmount, argsWithChainID("")),
		// sharding API
		// "shard_join": rpc.NewRPCFunc(h.ShardJoin, "gdata,cdata,sig"),
	}
}

func (h *rpcHandler) getShard(chainID string) (*MyNode, error) {
	var shard *MyNode
	//var err error
...
//else {
//		app := h.node.MainShard.Application.(*MyApp)
...
func (h *rpcHandler) Status(chainID string) (types.RPCResult, error) {
	...
	latestHeight := shard.Dngine.Height()
	if latestHeight != 0 {
		_, latestBlockMeta = shard.Dngine.GetBlock(latestHeight)
	...
	return &types.ResultStatus{
		NodeInfo:          shard.Dngine.GetNodeInfo(),
		PubKey:            shard.Dngine.PrivValidator().PubKey,
...
func (h *rpcHandler) GetAccountAmount(address string) (GetAccountAmountRes, error) {
	res := GetAccountAmountRes{
		Amount:managedState.accounts[address],
	}
	return res, nil
}
...
func (h *rpcHandler) BlockchainInfo(chainID string, minHeight, maxHeight int) (types.RPCResult, error) {
	...
	blockStoreHeight := shard.Dngine.Height()
	if maxHeight == 0 {
	...
	for height := maxHeight; height >= minHeight; height-- {
		_, blockMeta := shard.Dngine.GetBlock(height)
...
func (h *rpcHandler) DumpConsensusState(chainID string) (types.RPCResult, error) {
	...
	res.RoundState, res.PeerRoundStates = shard.Dngine.GetConsensusStateInfo()
	return &res, nil
}
...
func (h *rpcHandler) UnconfirmedTxs(chainID string) (types.RPCResult, error) {
	...
	res.Txs = shard.Dngine.GetUnconfirmedTxs()
	res.N = len(res.Txs)
	return &res, nil
}
...
func (h *rpcHandler) NumUnconfirmedTxs(chainID string) (types.RPCResult, error) {
	...
	return &types.ResultUnconfirmedTxs{N: shard.Dngine.GetNumUnconfirmedTxs(), Txs: nil}, nil
}
...
func (h *rpcHandler) UnsafeFlushMempool(chainID string) (types.RPCResult, error) {
	...
	shard.Dngine.FlushMempool()
	return &types.ResultUnsafeFlushMempool{}, nil
}

func (h *rpcHandler) BroadcastTx(chainID string, tx []byte) (*types.ResultBroadcastTx, error) {
	shard, err := h.getShard(chainID)
	...
	if err := shard.Dngine.BroadcastTx(tx); err != nil {
		return nil, err
	}
	res := &types.ResultBroadcastTx{Code: 0}
	return res, nil
}
...
func (h *rpcHandler) BroadcastTxCommit(chainID string, tx []byte) (types.RPCResult, error) {
	...
	if err := shard.Dngine.BroadcastTxCommit(tx); err != nil {
		return nil, err
	}
...
func (h *rpcHandler) Validators(chainID string) (types.RPCResult, error) {
	...
	height, vs := shard.Dngine.GetValidators()
...
func (h *rpcHandler) ZaSurveillance(chainID string) (types.RPCResult, error) {
	...
	if bcHeight >= 2 {
		...
		eBlock, _ := shard.Dngine.GetBlock(bcHeight)
		endTime := eBlock.Header.Time
		sBlock, _ := shard.Dngine.GetBlock(startHeight)
		...
		for h := startHeight + 1; h < bcHeight; h++ {
			block, _ := shard.Dngine.GetBlock(h)
	...
	_, vals := shard.Dngine.GetValidators()

	res := types.ResultSurveillance{
		Height:        bcHeight,
		NanoSecsPerTx: time.Duration(txAvg),
		Addr:          h.node.NodeInfo().RemoteAddr,
		IsValidator:   shard.Dngine.IsNodeValidator(h.node.NodeInfo().PubKey),
		NumValidators: len(vals),
		NumPeers:      shard.Dngine.GetNumPeers(),
...
func (h *rpcHandler) NetInfo(chainID string) (types.RPCResult, error) {
	...
	res.Listening, res.Listeners, res.Peers = shard.Dngine.GetP2PNetInfo()
	return &res, nil
}
...
func (h *rpcHandler) Blacklist(chainID string) (types.RPCResult, error) {
	...
	return &types.ResultRefuseList{Result: shard.Dngine.GetBlacklist()}, nil
}
...
func (h *rpcHandler) RequestSpecialOP(chainID string, tx []byte) (types.RPCResult, error) {
	...
	if err := shard.Dngine.ProcessSpecialOP(tx); err != nil {
...
func (h *rpcHandler) VoteSpecialOP(chainID string, tx []byte) (types.RPCResult, error) {
	...
	res, err := shard.Dngine.CheckSpecialOp(cmd)
	if err != nil {
		return &types.ResultRequestSpecialOP{
			Code: types.CodeType_InternalError,
			Log:  err.Error(),
...
//		return nil, err
//	}
//	shard.Dngine.BroadcastTxCommit(tx)
// }
